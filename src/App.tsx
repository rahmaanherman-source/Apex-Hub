...
import { Activity, AlertTriangle, ArrowUpRight, Bot, Boxes, CheckCircle2, Circle, Cloud, CreditCard, Database, ExternalLink, Github, Globe2, HardDrive, LayoutDashboard, LockKeyhole, Play, Search, Server, Settings, ShieldCheck, ShoppingBag, Terminal, WalletCards, Zap } from 'lucide-react';
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        <header className="topbar">
          <div><p className="eyebrow">APEX / KERNEL</p><h1>Command Center</h1></div>
          <div className="top-actions">
            <div className="search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search providers, capabilities…" /></div>
        ...
        <section className="hero-panel">
          ...
          <div className="hero-actions"><button className="primary-button" onClick={() => document.getElementById('provider-inventory')?.scrollIntoView({ behavior: 'smooth' })}><Play size={17} /> RUN CONNECTOR AUDIT</button><button className="secondary-button" onClick={() => openExternal('https://github.com/rahmaanherman-source/Apex-Hub')}><Github size={17} /> REPOSITORY</button></div>
        </section>
